...
namespace gdwg {
	...
	public:
		...
		struct value_type {
			...
			N to;
			E weight;
			//Added this so its easier for me test
	...
	private:
		...
		/*
		Case: Move Graph A to Graph B. Graph B has A's values now
		Graph A's iterators are not invalid, they now are essentially
		Graph B's iterators. We need this bottom function, so we have unique ids
		for each graph which is passed on to its iterators, this function is useful
		in the case where the iterator of "Graph A" is used to modify Graph B, since
		the ID values of Graph B are now Graph A's, whilst Graph A's ID values are refreshed
		*/
		static long ID ()
		{
			static long ID = 0;
		...
		std::map<Node,std::set<Edge,EdgeComparator>,NodeComparator> Graph_;
		long _id;
	};
	...
	class graph<N,E>::iterator {
		...
		public:
			...
			auto operator++() -> iterator& {
				...
				outer_--;
				middle_--;
				inner_ = middle_->weights_.end();
				end_ = true; //used for iterator erase edge case
				return *this;
			}
		...
		private:
			...
			, middle_iter middle
			, inner_iter inner)
			: pointee_(&pointee) ,id_(id), end_(end)
			  , outer_(outer), middle_(middle), inner_(inner) {}

			std::map<Node,std::set<Edge,EdgeComparator>,NodeComparator> const *pointee_ = nullptr;
			long id_; //I use cmp_ to check if iterators are valid when we invoke erase_edge
			bool end_;
			outer_iter outer_;
			middle_iter middle_;
			inner_iter inner_;
	...
	/*
		Please note when marking style, I understand doing : x(y) is the new way
		but I find it difficult to read, so I've stuck with this (other than explicit iterator)
	*/
	template<typename N, typename E>
	graph<N,E>::graph(){
	...
	graph<N,E>::graph(graph&& other) noexcept {
		...
		std::swap(_id, other._id); //Swapping IDS are essential
		other.clear();
	}
	...
	auto graph<N,E>::operator=(graph const& other) -> graph& {
		clear();
		_id = 0; //completely new graph, needs to be refreshed
		_id = ID();
		for(auto &x : other.Nodes_) {
	...
	auto graph<N,E>::replace_node(N const& old_data, N const& new_data) -> bool
	{
		...
		//I ensure all ingoing and outgoing edges are removed
	...
	auto graph<N,E>::merge_replace_node(N const& old_data, N const& new_data) -> void
	{
		...
		//Ingoing and outgoing edges are modified!
	...
	auto graph<N,E>::erase_edge(iterator i, iterator s) -> iterator {
		if(i.id_ != s.id_) { //Here, they must be of same id type
			return end();
		}
		if (i == s) {
			return i;
		...
		//O(1)*d where d is the distance between s and i
		// O(d)
		return i;
	}
	...
	template<typename N, typename E>
	[[nodiscard]] auto graph<N,E>::find(N const& src, N const& dst, E const& weight) const -> iterator {
		...
		return iterator(Graph_,_id,false,outer, middle, inner);
	}
