...
namespace gdwg {
	...
	class graph<N,E>::iterator {
		...
		public:
			...
			auto operator--() -> iterator& {
				if (inner_ == middle_->weights_.begin()) {
					if (middle_ == outer_->second.begin()) {
						if (outer_ == (*Accessor)->Graph_.begin()) {
			...
			auto operator==(iterator const& other) const -> bool {
				//Check if its of the same address
		...
		private:
			friend class graph;
			explicit iterator(std::shared_ptr<graph<N,E>*> pointee
			, outer_iter outer
			, middle_iter middle
			, inner_iter inner)
			...
			//this is const so we can use map of edges content with const_iterators
			//Its a pointer to a shared_ptr, in this case its pointee_ which is a shared_ptr
			<graph<N,E>> const *Accessor;
	...
	graph<N,E>::graph(graph const& other) {
		...
		for (auto y = other.begin(); y != other.end(); ++y) {
			insert_edge((*y).from, (*y).to, (*y).weight);
		}
	}
	template<typename N, typename E>
	auto graph<N,E>::operator=(graph const& other) -> graph& {
		...
		for (auto y = other.begin(); y != other.end(); ++y) {
			insert_edge((*y).from, (*y).to, (*y).weight); //e steps
		}
		return *this;
	}
	...
	auto graph<N,E>::erase_edge(iterator i) -> iterator {
		...
		++tmp;
		tmp = tmp == end() ? end() : tmp;
		//erase(Const Iterator I, Const iterator S)
		//Is a funciton that is actually meant to
		//delete all the items from I to S
		//It just so happens that, it returns a non const iterator to
	...
	[[nodiscard]] auto graph<N,E>::find(N const& src, N const& dst, E const& weight) -> iterator {
		...
		return iterator(GraphAddress_,outer, middle, inner);
	}
